...
export function getMarket(zip) {
 if (!zip) return null;
 const row = db.prepare('SELECT zip,county,city,latitude,longitude,population,income,home_value AS homeValue,owner_occupied AS ownerOccupied,median_year_built AS medianYearBuilt,luxury_share AS luxuryShare,data_year AS dataYear FROM markets WHERE zip=?').get(zip);
 return row ? { ...row, ...scoreMarket(row) } : null;
}

const propertySelect = `SELECT id,lookup_key AS lookupKey,provider_id AS providerId,parcel_id AS parcelId,address,city,state,zip,county,latitude,longitude,property_type AS propertyType,bedrooms,bathrooms,square_feet AS squareFeet,lot_square_feet AS lotSquareFeet,year_built AS yearBuilt,last_sale_date AS lastSaleDate,last_sale_price AS lastSalePrice,assessed_value AS assessedValue,estimated_value AS estimatedValue,owner_occupied AS ownerOccupied,flood_zone AS floodZone,flood_zone_subtype AS floodZoneSubtype,sfha,base_flood_elevation AS baseFloodElevation,sources_json AS sourcesJson,raw_json AS rawJson,fetched_at AS fetchedAt,expires_at AS expiresAt FROM properties`;

function hydrateProperty(row) {
 if (!row) return null;
 const property = { ...row, ownerOccupied: row.ownerOccupied == null ? null : Boolean(row.ownerOccupied), sfha: row.sfha == null ? null : Boolean(row.sfha), sources: JSON.parse(row.sourcesJson || '[]') };
...
const savePropertyStatement = db.prepare(`INSERT INTO properties(lookup_key,provider_id,parcel_id,address,city,state,zip,county,latitude,longitude,property_type,bedrooms,bathrooms,square_feet,lot_square_feet,year_built,last_sale_date,last_sale_price,assessed_value,estimated_value,owner_occupied,flood_zone,flood_zone_subtype,sfha,base_flood_elevation,sources_json,raw_json,fetched_at,expires_at)
VALUES(@lookupKey,@providerId,@parcelId,@address,@city,@state,@zip,@county,@latitude,@longitude,@propertyType,@bedrooms,@bathrooms,@squareFeet,@lotSquareFeet,@yearBuilt,@lastSaleDate,@lastSalePrice,@assessedValue,@estimatedValue,@ownerOccupied,@floodZone,@floodZoneSubtype,@sfha,@baseFloodElevation,@sourcesJson,@rawJson,@fetchedAt,@expiresAt)
ON CONFLICT(lookup_key) DO UPDATE SET provider_id=excluded.provider_id,parcel_id=excluded.parcel_id,address=excluded.address,city=excluded.city,state=excluded.state,zip=excluded.zip,county=excluded.county,latitude=excluded.latitude,longitude=excluded.longitude,property_type=excluded.property_type,bedrooms=excluded.bedrooms,bathrooms=excluded.bathrooms,square_feet=excluded.square_feet,lot_square_feet=excluded.lot_square_feet,year_built=excluded.year_built,last_sale_date=excluded.last_sale_date,last_sale_price=excluded.last_sale_price,assessed_value=excluded.assessed_value,estimated_value=excluded.estimated_value,owner_occupied=excluded.owner_occupied,flood_zone=excluded.flood_zone,flood_zone_subtype=excluded.flood_zone_subtype,sfha=excluded.sfha,base_flood_elevation=excluded.base_flood_elevation,sources_json=excluded.sources_json,raw_json=excluded.raw_json,fetched_at=excluded.fetched_at,expires_at=excluded.expires_at`);
...
export function saveProperty(property, ttlHours = 168) {
 ...
 savePropertyStatement.run({
  ...property, ownerOccupied: property.ownerOccupied == null ? null : Number(Boolean(property.ownerOccupied)),
  sfha: property.sfha == null ? null : Number(Boolean(property.sfha)), sourcesJson: JSON.stringify(property.sources || []),
  rawJson: JSON.stringify(property.raw || {}), fetchedAt: now.toISOString(), expiresAt: expires.toISOString()
 });
